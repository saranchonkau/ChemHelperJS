...
class Grid extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.data !== this.props.data) {
      this.setState({
        data: this.props.data.map(row => ({ rowId: v4(), ...row })),
      });
    }
  }
  ...
  handleShiftKeyUp = event =>
    event.keyCode === 16 && this.setState({ shiftPressed: false });
  ...
  sortColumn = columnIndex => {
    const currentColumnSort = this.state.sortModel.find(
      model => model.columnIndex === columnIndex,
    );
    const nextSortType = this.getNextSortType(
      get(currentColumnSort, 'sort', SortTypes.NONE),
    );
    ...
    if (this.state.shiftPressed) {
      if (currentColumnSort) {
        const filteredSortModel = this.state.sortModel.filter(
          model => model.columnIndex !== columnIndex,
        );
    ...
    const finalSortModel = updatedSortModel.filter(
      model => model.sort !== SortTypes.NONE,
    );
  ...
  getSortedData = () => {
    return [...this.state.sortModel].reverse().reduce(
      (data, currentModel) => {
        const { field } = this.props.options.columnDefs[
          currentModel.columnIndex
        ];
        return data.sort(
          this.getComparatorBySortType(currentModel.sort, field),
        );
      },
      [...this.state.data],
    );
  ...
  getRowData = () =>
    this.state.sortModel.length ? this.getSortedData() : this.state.data;
  ...
  getGridWidth = () =>
    this.props.options.columnDefs.reduce(
      (sum, current) => sum + current.width,
      0,
    );
  ...
  render() {
    const data = this.state.sortModel.length
      ? this.getSortedData()
      : this.state.data;
    return (
